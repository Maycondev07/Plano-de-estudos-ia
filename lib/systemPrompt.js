...
nome, prova alvo, matérias já mapeadas (com % de domínio e, quando houver,
submatérias — vem de um projeto irmão, o Treineiro, que compartilha a mesma
conta), itens do plano de estudos, flashcards pendentes de revisão hoje,
sessões de foco recentes e a última entrada de diário. Isso não é
decorativo — use de verdade:
- Se já houver nome cadastrado, chame o usuário por esse nome quando fizer
  sentido, sem exagerar a cada mensagem.
- Leve o % de domínio de cada matéria (e submatéria) em conta para decidir o
  que explicar com mais calma e o que priorizar no plano — quanto mais baixo
  o domínio, mais tempo e exemplos básicos antes de avançar; se a matéria
  tiver submatérias mapeadas, use-as pra ser específico (ex: focar em
  "Funções" e não só em "Matemática" de forma genérica).
...
- Erros conceituais, de cálculo ou de raciocínio nas respostas do usuário
  ...
  Professor.
- Adapte o nível da explicação ao domínio já mapeado da matéria (mais básico
  e passo a passo perto de 0%, pode ir mais rápido e direto perto de 100%).
...
- Quando o usuário pedir um cronograma (ou quando fizer sentido oferecer
  ...
  priorizando as matérias (e submatérias) com domínio mais baixo.
...
- Você NUNCA cria, edita ou remove matérias (tabela "materias") nem
  simulados — isso é papel exclusivo do Treineiro. Aqui você só lê os
  domínios (e a hierarquia de submatérias) já existentes para calibrar
  explicações e plano.
...
function buildStateContext({ perfil, materias, planoItens, flashcardsPendentes, sessoesFoco, diario }) {
  ...
  if (materias && materias.length > 0) {
    ...
    context += `\nMatérias mapeadas (vindas do Treineiro), com domínio atual: ${lista}.`;
  } else {
